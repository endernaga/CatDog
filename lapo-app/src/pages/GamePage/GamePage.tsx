...
export const GamePage = () => {
  ...
  const isSelect = (v: string, index: number) => {
    return index === 0 ? v === firstItemChoice : v === secondItemChoice;
  };
  ...
  const checkItem = (
    ...
    if (choice === petSex) {
      gameRef.current
        ?.querySelector(`#cart-${index}`)
        ?.classList.add("game__cart--success");
    } else {
      gameRef.current
        ?.querySelector(`#cart-${index}`)
        ?.classList.add("game__cart--loss");
    }
  };

  const startStyleReset = () => {
    checkboxes.forEach((item) => ((item as HTMLElement).style.display = "flex"));
    infos.forEach((item) => ((item as HTMLElement).style.display = "none"));
    buttons.forEach((item) => ((item as HTMLElement).style.display = "none"));
    carts.forEach((item) => ((item as HTMLElement).className = "game__cart"));
  };

  useEffect(() => {
    if (!isRound) {
      checkItem(firstItemChoice, firstPet?.sex, 0);
      checkItem(secondItemChoice, secondPet?.sex, 1);
      checkboxes.forEach((item) => (item as HTMLElement).style.display = "none");
      infos.forEach((item) => (item as HTMLElement).style.display = "flex");
      buttons.forEach((item) => (item as HTMLElement).style.display = "block");
    }
  }, [isRound]);
  ...
  const fetchPets = async () => {
    try {
      const data = await getPetsForGame();
        ...
        <>
          {isStartPageVisible && (
            <div className="game__start">
              <h3 className="game__start__title">
          ...
          {!isStartPageVisible && pets && (
            <div className="game__content">
              ...
              <div className="game__carts">
                {pets.map((pet, index) => (
                  <div className="game__cart" id={`cart-${index}`} key={index}>
                    <div
